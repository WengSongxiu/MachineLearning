...
​	One-hot编码又称为一位有效编码，如果离散变量的种类有M个，就采用M位二进制表示，M位中只有一位有效，代表一个种类。以性别为例：{‘男’，‘女’，‘未知’}
...
​	One-hot编码将不可排序的离散变量映射到欧式空间，保留了变量的等距特性。

​	如果离散变量的类别特别多，One-hot编码的维度会特别高，会造成存储上的消耗。为了节省空间，一方面可以采用稀疏编码的方式对编码后的变量进行存储，另一方面可以考虑先分箱，后编码。

- Dummy Variable编码（哑变量）

​	哑变量编码同样采用二进制编码的方式来表示变量的值，不同的是，如果离散变量的种类有M个，哑变量编码只用M-1维的二进制就可以表示M中可能出现的值。

​	哑变量编码跟One-hot编码一样，同样面临维度过高造成存储消耗问题，而且哑变量编码在不可排序变量的编码中，无法保留变量的等距特性。

| 类别取值 | Dummy Variable编码 |
| :------: | :----------------: |
|    男    |         10         |
|    女    |         01         |
|   未知   |         00         |

One-hot编码和Dummy Variable编码的优点：

​	- 离散变量数值化，使离散变量间的距离可计算；

​	- 增加了模型的非线性。

- Label编码

​	在离散变量中，可排序的变量数值化转换时，如果想保留等级大小关系，则需要用标签编码（Label）编码来完成。标签编码就是简单地将离散变量按照等级转化为数值的过程。没有实际的意义，也没有计算距离的要求。
...
​	WOE（Weight of Evidence）编码是评分卡中最常见的有监督编码。既可以对离散变量进行编码，也可以对连续变量进行编码。WOE编码就是对坏样本分布与好样本分布的比值进行对数变换的结果。WOE值可以表示变量的每个取值的预测能力。

​	WOE编码的好处：

	- 增加变量的可解释性。
	- 体现变量的预测能力。
	- 可以处理缺失值，将缺失值作为一个特征进行编码。

#### 3.2.3 WOE与One-hot的比较

​	WOE编码和One-hot编码的区别在于参数训练过程。在WOE编码的模型中，参数的一部分是在已知训练样本的基础上求得的WOE值再乘以模型训练得到的权重；而在One-hot编码的模型中，参数完全由模型训练得到。

​	在Logistics回归中采用WOE编码或One-hot编码对于模型预测效果没有差异，不存在WOE编码优于One-hot编码的现象。
